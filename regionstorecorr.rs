...
//!
//! The regions in a RegionStoreCorr may not have the same number of integers.
...
impl RegionStoreCorr {
    ...
    /// Return True if a RegionStoreCorr is a superset of all corresponding states in a StateStore.
    ...
    /// Return True if a RegionStoreCore is a superset of another RegionStoreCorr.
    /// Used in optimal regionstore calculations.
    pub fn is_superset(&self, other: &RegionStoreCorr) -> bool {
    ...
    /// Return an intersection of corresponding regions, of two RegionStoreCorrs.
    /// Used in optimal regionstore calculations.
    ...
    /// Used in optimal regionstore calculations.
    pub fn subset(&self, other: &RegionStoreCorr) -> bool {
        debug_assert!(self.len() == other.len());
    ...
    /// Subtract RegionStoreCore from another.
...
#[allow(dead_code)]
fn vec_rs_contains(avec: &Vec<RegionStoreCorr>, ars: &RegionStoreCorr) -> bool {
    for rsx in avec {
        if rsx == ars {
            return true;
        }
...
pub fn vec_rs_any_superset(rs_vec: &[RegionStoreCorr], reg_str: &RegionStoreCorr) -> bool {
    for regx in rs_vec {
        if regx.is_superset(reg_str) {
            return true;
        }
    }
    false
...
pub fn vec_rs_push_nosubs(rs_vec: &mut Vec<RegionStoreCorr>, reg_str: RegionStoreCorr) -> bool {
    // Check for supersets, which probably is an error
    if vec_rs_any_superset(rs_vec, &reg_str) {
        return false;
    }
...
/// result.
pub fn vec_rs_split_to_subsets(rs_vec: &Vec<RegionStoreCorr>) -> Vec<RegionStoreCorr> {
        ...
        for iny in (inx + 1)..rs_vec.len() {
            ...
            if let Some(int) = rs_vec[inx].intersection(&rs_vec[iny]) {
                ...
                for rsz in rs_vec[inx].subtract(&int).into_iter() {
                    vec_rs_push_nosubs(&mut tmp_vec, rsz);
                }

                for rsz in rs_vec[iny].subtract(&int).into_iter() {
                    vec_rs_push_nosubs(&mut tmp_vec, rsz);
                }
                vec_rs_push_nosubs(&mut tmp_vec, int);
            }
        }
    }
    ...
    for (iny, rsy) in rs_vec.iter().enumerate() {
        if !int_vec.contains(&iny) {
            vec_rs_push_nosubs(&mut ret_vec, rsy.clone());
        }
    }
            ...
            for rsx in rs_vec.iter() {
                if let Some(int) = rsy.intersection(rsx) {
                    ...
                    for rsz in rsy.subtract(&int).into_iter() {
                        vec_rs_push_nosubs(&mut next_pass, rsz);
                    }
                    // Add the intersection to the next_pass vector.
                    vec_rs_push_nosubs(&mut next_pass, int);
                    split = true;
                }
            } // next rsx
              // If no intersectiosn, add the fragment to the return vector.
            if !split {
                vec_rs_push_nosubs(&mut ret_vec, rsy);
            }
        } // next rsy
...
mod tests {
    ...
    #[test]
    fn test_vec_rs_split_by_partial_intersection() -> Result<(), String> {
        ...
        let rslt = vec_rs_split_to_subsets(&rs_vec);
    ...
    fn subtract() -> Result<(), String> {
        ...
        assert!(vec_rs_contains(&subt, &regstr3));
