...
mod tests {
    ...
    fn most_recent_result() -> Result<(), String> {
        ...
        let sta3 = SomeState::new_from_string("0x3")?;
        let sta4 = SomeState::new_from_string("0x4")?;
        let sta5 = SomeState::new_from_string("0x5")?;
        let sta6 = SomeState::new_from_string("0x6")?;
        let sta7 = SomeState::new_from_string("0x7")?;
        let sta8 = SomeState::new_from_string("0x8")?;
        let sta9 = SomeState::new_from_string("0x9")?;
    ...
    fn add_result_pn_one() -> Result<(), String> {
        let mut rslt_str = ResultStore::new(vec![SomeState::new_from_string("0x505")?]);

        let pn = rslt_str.add_result(SomeState::new_from_string("0x505")?);
    ...
    fn add_result_pn_two() -> Result<(), String> {
        let mut rslt_str = ResultStore::new(vec![SomeState::new_from_string("0x505")?]);
        let mut pn = rslt_str.add_result(SomeState::new_from_string("0x504")?);
        println!("Pn: {pn} results: {rslt_str}");
        assert!(pn == Pn::Two);

        pn = rslt_str.add_result(SomeState::new_from_string("0x505")?);
        println!("Pn: {pn} results: {rslt_str}");
        assert!(pn == Pn::Two);

        pn = rslt_str.add_result(SomeState::new_from_string("0x504")?);
        println!("Pn: {pn} results: {rslt_str}");
        assert!(pn == Pn::Two);
    ...
    fn add_result_pn_unpredictable() -> Result<(), String> {
        ...
        pn = rslt_str.add_result(SomeState::new_from_string("0x504")?); // two results, but out of order
        println!("Pn: {pn} results: {rslt_str}");
        assert!(pn == Pn::Unpredictable);

        // Test three different results.
        rslt_str = ResultStore::new(vec![SomeState::new_from_string("0x505")?]);
        pn = rslt_str.add_result(SomeState::new_from_string("0x504")?);
        println!("Pn: {pn} results: {rslt_str}");
        assert!(pn == Pn::Two);

        pn = rslt_str.add_result(SomeState::new_from_string("0x502")?); // two results, but out of order
        println!("Pn: {pn} results: {rslt_str}");
        assert!(pn == Pn::Unpredictable);
    ...
    fn add_result_misc() -> Result<(), String> {
        let mut rslt_str = ResultStore::new(vec![SomeState::new_from_string("0x500")?]);
        rslt_str.add_result(SomeState::new_from_string("0x501")?);
        rslt_str.add_result(SomeState::new_from_string("0x502")?);
        rslt_str.add_result(SomeState::new_from_string("0x503")?);
        rslt_str.add_result(SomeState::new_from_string("0x504")?);
        rslt_str.add_result(SomeState::new_from_string("0x505")?);
        rslt_str.add_result(SomeState::new_from_string("0x506")?);

        println!("results: {rslt_str}");
        assert!(*rslt_str.first() == SomeState::new_from_string("0x504")?);
        assert!(*rslt_str.second() == SomeState::new_from_string("0x505")?);
        assert!(*rslt_str.most_recent_result() == SomeState::new_from_string("0x506")?);
